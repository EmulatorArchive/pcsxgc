...
#define execI() { \
	u32 *code = (u32 *)psxMemRLUT[(psxRegs.pc) >> 16]; /*PSXM(psxRegs.pc);*/ \
        psxRegs.code = /*(code == NULL) ? 0 :*/ SWAP32p((u32*)(((u8*)code) + ((psxRegs.pc) & 0xffff))); \
 \
	debugI(); \
 \
...
// Subsets
void (*psxBSC[64])(u32 opcode);
void (*psxSPC[64])(u32 opcode);
void (*psxREG[32])(u32 opcode);
void (*psxCP0[32])(u32 opcode);
void (*psxCP2[64])();
void (*psxCP2BSC[32])();
...
*********************************************************/
void psxADDI(u32 opcode) 	{ if (!_Rt_) return; _rRt_ = _u32(_rRs_) + _Imm_ ; }		// Rt = Rs + Im 	(Exception on Integer Overflow)
void psxADDIU(u32 opcode) { if (!_Rt_) return; _rRt_ = _u32(_rRs_) + _Imm_ ; }		// Rt = Rs + Im
void psxANDI(u32 opcode) 	{ if (!_Rt_) return; _rRt_ = _u32(_rRs_) & _ImmU_; }		// Rt = Rs And Im
void psxORI(u32 opcode) 	{ if (!_Rt_) return; _rRt_ = _u32(_rRs_) | _ImmU_; }		// Rt = Rs Or  Im
void psxXORI(u32 opcode) 	{ if (!_Rt_) return; _rRt_ = _u32(_rRs_) ^ _ImmU_; }		// Rt = Rs Xor Im
void psxSLTI(u32 opcode) 	{ if (!_Rt_) return; _rRt_ = _i32(_rRs_) < _Imm_ ; }		// Rt = Rs < Im		(Signed)
void psxSLTIU(u32 opcode) { if (!_Rt_) return; _rRt_ = _u32(_rRs_) < ((u32)_Imm_); }		// Rt = Rs < Im		(Unsigned)

/*********************************************************
* Register arithmetic                                    *
* Format:  OP rd, rs, rt                                 *
*********************************************************/
void psxADD(u32 opcode)	{ if (!_Rd_) return; _rRd_ = _u32(_rRs_) + _u32(_rRt_); }	// Rd = Rs + Rt		(Exception on Integer Overflow)
void psxADDU(u32 opcode) 	{ if (!_Rd_) return; _rRd_ = _u32(_rRs_) + _u32(_rRt_); }	// Rd = Rs + Rt
void psxSUB(u32 opcode) 	{ if (!_Rd_) return; _rRd_ = _u32(_rRs_) - _u32(_rRt_); }	// Rd = Rs - Rt		(Exception on Integer Overflow)
void psxSUBU(u32 opcode) 	{ if (!_Rd_) return; _rRd_ = _u32(_rRs_) - _u32(_rRt_); }	// Rd = Rs - Rt
void psxAND(u32 opcode) 	{ if (!_Rd_) return; _rRd_ = _u32(_rRs_) & _u32(_rRt_); }	// Rd = Rs And Rt
void psxOR(u32 opcode) 	{ if (!_Rd_) return; _rRd_ = _u32(_rRs_) | _u32(_rRt_); }	// Rd = Rs Or  Rt
void psxXOR(u32 opcode) 	{ if (!_Rd_) return; _rRd_ = _u32(_rRs_) ^ _u32(_rRt_); }	// Rd = Rs Xor Rt
void psxNOR(u32 opcode) 	{ if (!_Rd_) return; _rRd_ =~(_u32(_rRs_) | _u32(_rRt_)); }// Rd = Rs Nor Rt
void psxSLT(u32 opcode) 	{ if (!_Rd_) return; _rRd_ = _i32(_rRs_) < _i32(_rRt_); }	// Rd = Rs < Rt		(Signed)
void psxSLTU(u32 opcode) 	{ if (!_Rd_) return; _rRd_ = _u32(_rRs_) < _u32(_rRt_); }	// Rd = Rs < Rt		(Unsigned)
...
void psxBGEZ(u32 opcode)   { RepZBranchi32(>=) }      // Branch if Rs >= 0
void psxBGEZAL(u32 opcode) { RepZBranchLinki32(>=) }  // Branch if Rs >= 0 and link
void psxBGTZ(u32 opcode)   { RepZBranchi32(>) }       // Branch if Rs >  0
void psxBLEZ(u32 opcode)   { RepZBranchi32(<=) }      // Branch if Rs <= 0
void psxBLTZ(u32 opcode)   { RepZBranchi32(<) }       // Branch if Rs <  0
void psxBLTZAL(u32 opcode) { RepZBranchLinki32(<) }   // Branch if Rs <  0 and link
...
void psxSRA(u32 opcode) { if (!_Rd_) return; _i32(_rRd_) = _i32(_rRt_) >> _Sa_; } // Rd = Rt >> sa (arithmetic)
...
void psxSRAV(u32 opcode) { if (!_Rd_) return; _i32(_rRd_) = _i32(_rRt_) >> _u32(_rRs_); } // Rd = Rt >> rs (arithmetic)
...
*********************************************************/
void psxLUI(u32 opcode) { if (!_Rt_) return; _u32(_rRt_) = psxRegs.code << 16; } // Upper halfword of Rt = Im

/*********************************************************
* Move from HI/LO to GPR                                 *
* Format:  OP rd                                         *
*********************************************************/
void psxMFHI(u32 opcode) { if (!_Rd_) return; _rRd_ = _rHi_; } // Rd = Hi
void psxMFLO(u32 opcode) { if (!_Rd_) return; _rRd_ = _rLo_; } // Rd = Lo

/*********************************************************
* Move to GPR to HI/LO & Register jump                   *
* Format:  OP rs                                         *
*********************************************************/
void psxMTHI(u32 opcode) { _rHi_ = _rRs_; } // Hi = Rs
void psxMTLO(u32 opcode) { _rLo_ = _rRs_; } // Lo = Rs

/*********************************************************
* Special purpose instructions                           *
* Format:  OP                                            *
*********************************************************/
void psxBREAK(u32 opcode) {
	// Break exception - psx rom doens't handles this
}

void psxSYSCALL(u32 opcode) {
	psxRegs.pc -= 4;
	psxException(0x20, branch);
}

void psxRFE(u32 opcode) {
//	SysPrintf("psxRFE\n");
...
void psxBEQ(u32 opcode) {	RepBranchi32(==) }  // Branch if Rs == Rt
void psxBNE(u32 opcode) {	RepBranchi32(!=) }  // Branch if Rs != Rt

/*********************************************************
* Jump to target                                         *
* Format:  OP target                                     *
*********************************************************/
void psxJ(u32 opcode)   {               doBranch(_JumpTarget_); }
void psxJAL(u32 opcode) {	_SetLink(31); doBranch(_JumpTarget_); }

/*********************************************************
* Register jump                                          *
* Format:  OP rs, rd                                     *
*********************************************************/
void psxJR(u32 opcode)   {                 doBranch(_u32(_rRs_)); }
void psxJALR(u32 opcode) {
	u32 temp = _u32(_rRs_);
...
void psxSB(u32 opcode) { psxMemWrite8 (_oB_, _u8 (_rRt_)); }
void psxSH(u32 opcode) { psxMemWrite16(_oB_, _u16(_rRt_)); }
void psxSW(u32 opcode) { psxMemWrite32(_oB_, _u32(_rRt_)); }

u32 SWL_MASK[4] = { 0xffffff00, 0xffff0000, 0xff000000, 0 };
u32 SWL_SHIFT[4] = { 24, 16, 8, 0 };

void psxSWL(u32 opcode) {
	u32 addr = _oB_;
	u32 shift = addr & 3;
	u32 mem = psxMemRead32(addr & ~3);
...
* Format:  OP rt, fs                                     *
*********************************************************/
void psxMFC0(u32 opcode) { if (!_Rt_) return; _i32(_rRt_) = (int)_rFs_; }
void psxCFC0(u32 opcode) { if (!_Rt_) return; _i32(_rRt_) = (int)_rFs_; }
...
void psxMTC0(u32 opcode) { MTC0(_Rd_, _u32(_rRt_)); }
void psxCTC0(u32 opcode) { MTC0(_Rd_, _u32(_rRt_)); }

/*********************************************************
* Unknow instruction (would generate an exception)       *
* Format:  ?                                             *
*********************************************************/
void psxNULL(u32 opcode) { 
#ifdef PSXCPU_LOG
...
void gteNULL() { 
#ifdef PSXCPU_LOG
	PSXCPU_LOG("gte: Unimplemented op %x\n", psxRegs.code);
#endif
}

void psxSPECIAL(u32 opcode) {
	psxSPC[_Funct_](opcode);
}

void psxREGIMM(u32 opcode) {
	psxREG[_Rt_](opcode);
}

void psxCOP0(u32 opcode) {
	psxCP0[_Rs_](opcode);
}

void psxCOP2() {
	psxCP2[_fFunct_(psxRegs.code)]();
}

void psxBASIC() {
	psxCP2BSC[_fRs_(psxRegs.code)]();
}

void psxHLE() {
	if ((psxRegs.code & 0x3ffffff) == (psxRegs.code & 0x7)) {
		psxHLEt[psxRegs.code & 0x7]();
	} else {
		// somebody else must have written to current opcode for this to happen!!!!
		psxHLEt[0](); // call dummy function
	}
...
void (*psxCP2BSC[32])() = {
	gteMFC2, gteNULL, gteCFC2, gteNULL, gteMTC2, gteNULL, gteCTC2, gteNULL,
	gteNULL, gteNULL, gteNULL, gteNULL, gteNULL, gteNULL, gteNULL, gteNULL,
	gteNULL, gteNULL, gteNULL, gteNULL, gteNULL, gteNULL, gteNULL, gteNULL,
	gteNULL, gteNULL, gteNULL, gteNULL, gteNULL, gteNULL, gteNULL, gteNULL
};
